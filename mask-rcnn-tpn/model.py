...
class PN(nn.Module):
    def __init__(self, in_channels, in_depth):
        ...
        self.bn2 = nn.BatchNorm3d(9, eps=0.001)
        
    def forward(self, x):
        xn = x.data.cpu().numpy()
        x = self.conv1(x)
        xn1 = x.data.cpu().numpy()
        x = self.bn1(x)
        xb1 = x.data.cpu().numpy()
        x = self.relu(x)
        xr1 = x.data.cpu().numpy()
        x = self.conv2(x)
        xn2 = x.data.cpu().numpy()
        x = self.pool3(x)
        diff1 = (xn - xn1[:,0::4,:,:,:]) ** 2
        diff2 = (xn - xb1[:,0::4,:,:,:]) ** 2
        diff3 = (xn - xr1[:,0::4,:,:,:]) ** 2
        diff4 = (xn - xn2) ** 2
        print('diff conv1', np.sum(diff1))
        print('diff bn1', np.sum(diff2))
        print('diff relu1', np.sum(diff3))
        print('diff conv2', np.sum(diff4))
...
class MaskRCNN(nn.Module):
    ...
    def build(self, config):
        ...
        self.pn = PN(in_channels=3, in_depth=2)
    ...
    def train_epoch(self, datagenerator, optimizer, steps):
        ...
        for i, inputs in enumerate(datagenerator):
            ...
            if prev_image is None:
                prev_image = images0
            images = torch.cat((prev_image.view(1, C, 1, H, W), images0.view(1, C, 1, H, W)), 2)
            prev_image = images0
