...
def unique1d(tensor):
    ...
    print(first_element.size())
    print(unique_bool.size())
...
class PN(nn.Module):
    ...
    def forward(self, x):
        with open('image_in.pkl', 'wb') as f:
            pickle.dump(x[:,:,0,:,:].data.cpu().numpy(), f)
        x0 = x.clone() # (1, 3, 5, 1024, 1024)
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.conv2(x)
        x = self.relu(x) # (1, 3, 1, 1024, 1024)

        x = torch.cat((x, x0[:,:,1:,:,:]), dim=2)
        x = self.conv3(x)
        x = self.bn2(x)
        x = self.conv4(x)
        x = self.relu(x)
        ...
        with open('tpn_conv1_weight.pkl', 'wb') as f:
            pickle.dump(self.conv1.weight.data.cpu().numpy(), f)
        with open('tpn_conv3_weight.pkl', 'wb') as f:
            pickle.dump(self.conv3.weight.data.cpu().numpy(), f)
        with open('tpn_conv6_weight.pkl', 'wb') as f:
            pickle.dump(self.conv6.weight.data.cpu().numpy(), f)
        with open('tpn_out.pkl', 'wb') as f:
...
class MaskRCNN(nn.Module):
    ...
    def build(self, config):
        ...
        self.pn = PN(in_channels=3, in_depth=5)
    ...
    def detect(self, images):
        ...
        print(self.feedback_boxes.size())
        # Convert to numpy
    ...
    def predict(self, input, mode):
        ...
        if len(molded_images.size()) > 4:
            prenet_out = self.pn(molded_images)
        else:
