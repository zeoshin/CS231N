...
class SamePad3d(nn.Module):
    ...
    def forward(self, input):
        ...
        in_height = input.size()[4]
        out_depth = math.ceil(float(in_depth) / float(self.stride[0]))
        out_width = math.ceil(float(in_width) / float(self.stride[1]))
        out_height = math.ceil(float(in_height) / float(self.stride[2]))
        pad_along_depth = ((out_depth - 1) * self.stride[0] +
                           self.kernel_size[0] - in_depth)
        pad_along_width = ((out_width - 1) * self.stride[1] +
                           self.kernel_size[1] - in_width)
        pad_along_height = ((out_height - 1) * self.stride[2] +
                            self.kernel_size[2] - in_height)
        pad_front = math.floor(pad_along_depth / 2)
        pad_left = math.floor(pad_along_width / 2)
        pad_top = math.floor(pad_along_height / 2)
        pad_back = pad_along_depth - pad_front
        pad_right = pad_along_width - pad_left
        pad_bottom = pad_along_height - pad_top
        return F.pad(input, (pad_left, pad_right, pad_top, pad_bottom, pad_front, pad_back), 'constant', 0)
...
class TPN(nn.Module):
    def __init__(self, in_channels, in_depth, is_residual):
        super(TPN, self).__init__()
        self.is_residual = is_residual
        self.conv1 = nn.Conv3d(in_channels, 12, (3, 5, 5), 1, (1, 2, 2))
        #self.conv1.weight.data /= 700
        #self.conv1.weight.data[0:4,0,1,2,2] = 1
        #self.conv1.weight.data[4:8,1,1,2,2] = 1
        #self.conv1.weight.data[8:12,2,1,2,2] = 1
        self.conv2 = nn.Conv3d(12, 3, (in_depth, 7, 7), 1, (0, 3, 3))
        #self.conv2.weight.data /= 10000
        #self.conv2.weight.data[0,0:4,0,3,3] = 0.25
        #self.conv2.weight.data[1,4:8,0,3,3] = 0.25
        #self.conv2.weight.data[2,8:12,0,3,3] = 0.25
        self.bn1 = nn.BatchNorm3d(12, eps=0.001)

        self.conv3 = nn.Conv3d(in_channels, 12, (3, 5, 5), 1, (1, 2, 2))
        #self.conv3.weight.data /= 1400
        #self.conv3.weight.data[0:4,0,1,2,2] = 1
        #self.conv3.weight.data[4:8,1,1,2,2] = 1
        #self.conv3.weight.data[8:12,2,1,2,2] = 1
        self.conv4 = nn.Conv3d(12, 3, (in_depth, 7, 7), 1, (0, 3, 3))
        #self.conv4.weight.data /= 20000
        #self.conv4.weight.data[0,0:4,0,3,3] = 0.25
        #self.conv4.weight.data[1,4:8,0,3,3] = 0.25
        #self.conv4.weight.data[2,8:12,0,3,3] = 0.25
        self.bn2 = nn.BatchNorm3d(12, eps=0.001)

        self.conv5 = nn.Conv3d(in_channels, 12, (3, 5, 5), 1, (1, 2, 2))
        #self.conv5.weight.data /= 7000
        #self.conv5.weight.data[0:4,0,1,2,2] = 1
        #self.conv5.weight.data[4:8,1,1,2,2] = 1
        #self.conv5.weight.data[8:12,2,1,2,2] = 1
        self.conv6 = nn.Conv3d(12, 3, (in_depth, 7, 7), 1, (0, 3, 3))
        #self.conv6.weight.data /= 100000
        #self.conv6.weight.data[0,0:4,0,3,3] = 0.25
        #self.conv6.weight.data[1,4:8,0,3,3] = 0.25
        #self.conv6.weight.data[2,8:12,0,3,3] = 0.25
        self.bn3 = nn.BatchNorm3d(12, eps=0.001)
    ...
    def forward(self, x):
        ...
        x = self.bn1(x)
        x = self.conv2(x)
        x = self.relu(x) # (1, 3, 1, 1024, 1024)

        x = torch.cat((x, x_orig[:,:,1:,:,:]), dim=2)
        x = self.conv3(x)
        x = self.bn2(x)
        x = self.conv4(x)
        x = self.relu(x)
        ...
        x = self.conv6(x)
        x = self.relu(x)   
#        print("TPN output size before view {}".format(x.size()))     
        x = x.view(1, 3, 1024, 1024)
#        print("TPN output size {}".format(x.size()))
        if self.is_residual:
            return x + x_orig[:,:,0,:,:]
        return x
...
class SPN(nn.Module):
    def __init__(self, in_channels, in_depth):
        super(SPN, self).__init__()
        self.conv1 = nn.Conv3d(in_channels, 15, (3, 13, 13), 1, (1, 6, 6))
        #self.conv1.weight.data /= 1000
        #self.conv1.weight.data[0:5,0,1,6,6] = 1 
        #self.conv1.weight.data[5:10,1,1,6,6] = 1
        #self.conv1.weight.data[10:15,2,1,6,6] = 1
        self.conv2 = nn.Conv3d(15, 9, (3, 3, 3), 1, (1, 1, 1))
        #self.conv2.weight.data /= 100000
        #self.conv2.weight.data[0:3,0:5,1,1,1] = 0.2
        #self.conv2.weight.data[3:6,5:10,1,1,1] = 0.2
        #self.conv2.weight.data[6:9,10:15,1,1,1] = 0.2
        self.conv3 = nn.Conv3d(9, 3, (in_depth, 9, 9), 1, (0, 4, 4))
        #self.conv3.weight.data /= 100000
        #self.conv3.weight.data[0,0:3,0,4,4] = 1.0/3
        #self.conv3.weight.data[1,3:6,0,4,4] = 1.0/3
        #self.conv3.weight.data[2,6:9,0,4,4] = 1.0/3
        self.bn1 = nn.BatchNorm3d(9, eps=0.001)
        self.relu = nn.ReLU(inplace=True)  
    def forward(self, x):
        x_orig = x
        x_in = x.data.cpu().numpy()
        x = self.conv1(x)
        x = self.conv2(x)
        x_conv2 = x.data.cpu().numpy()
        x = self.bn1(x)
        #x = self.relu(x)
        x_rl = x.data.cpu().numpy()
        x = self.conv3(x)
        x_conv3 = x.data.cpu().numpy()
        #diff2 = (x_in - x_conv2[:,0::3,:,:,:]) ** 2
        #diff4 = (x_in - x_rl[:,0::3,:,:,:]) ** 2
        #diff5 = (x_in - x_conv3) ** 2
        #print('diff conv2', np.mean(diff2))
        #print('diff relu1', np.mean(diff4))
        #print('diff conv3', np.mean(diff5))
        x = x.view(1, 3, 1024, 1024)
        # comment here to enable residual and uncomment weights initialization
        x = self.relu(x) + x_orig[:,:,0,:,:]
        return x
...
def proposal_layer(inputs, proposal_count, nms_threshold, anchors, feedback_boxes, config=None):
    ...
    boxes = boxes[keep, :]
    # RNN additional
    # boxes corresponding to scaled, padded image
    # if feedback_boxes is not None:
        # boxes = torch.cat((boxes, feedback_boxes), 0)
...
class MaskRCNN(nn.Module):
    ...
    def build(self, config):
        ...
        self.tpn1 = TPN(in_channels=3, in_depth=5, is_residual=True)
        self.tpn2 = TPN(in_channels=3, in_depth=5, is_residual=False)
        self.spn = SPN(in_channels=3, in_depth=2)
    ...
    def detect(self, images):
        ...
        if molded_images.shape[0] > 1:
            molded_images = np.expand_dims(molded_images.transpose(3,0,1,2),0)
        ...
        # RNN additional
        #self.feedback_boxes = detections.squeeze(0)
        #self.feedback_boxes = self.feedback_boxes[:, :4]
        # Convert to numpy
    ...
    def predict(self, input, mode):
        ...
        if len(molded_images.size()) > 4:
            N, C, D, H, W = molded_images.size()
            print("molded_image size {}".format(molded_images.size()))
            molded_images1 = molded_images[:,:,:5,:,:]
            molded_images2 = molded_images[:,:,5:,:,:]
            tpn_out1 = self.tpn1(molded_images1).view(1,C,1,H,W)
            tpn_out2 = self.tpn2(molded_images2).view(1,C,1,H,W)
            tpn_out2_flip = Variable(torch.Tensor((tpn_out2.cpu().data.numpy()[::-1].copy())))
            spn_in = torch.cat((tpn_out1.cuda(), tpn_out2_flip.cuda()), 2)
            prenet_out = self.spn(spn_in)
        else:
            prenet_out = molded_images
        # Feature extraction
        print("prenet_out size {}".format(prenet_out.size()))
        [p2_out, p3_out, p4_out, p5_out, p6_out] = self.fpn(prenet_out)
        ...
        rpn_rois = proposal_layer([rpn_class, rpn_bbox],
                                 ...
                                 feedback_boxes=self.feedback_boxes,
                                 config=self.config)
        if self.feedback_boxes is not None:
            self.feedback_boxes = None
        if mode == 'inference':
            # Network Heads
    ...
    def train_model(self, train_dataset, val_dataset, learning_rate, epochs, layers):
        ...
        layer_regex = {
            ...
            "4+": r"(spn.*)|(tpn.*)|(tpn1.*)|(tpn2.*)|(fpn.C4.*)|(fpn.C5.*)|(fpn.P5\_.*)|(fpn.P4\_.*)|(fpn.P3\_.*)|(fpn.P2\_.*)|(rpn.*)|(classifier.*)|(mask.*)",
    ...
    def train_epoch(self, datagenerator, optimizer, steps):
        ...
        for i, inputs in enumerate(datagenerator):
            images0 = inputs[0]
            N, C, H, W = images0.size()
            vid_frames = [images0.view(1,C,1,H,W)] + vid_frames
            if len(vid_frames) < 10:
                continue
            step += 1
            batch_count += 1
            images = torch.cat((
                vid_frames[0], vid_frames[1], vid_frames[2],
                vid_frames[3], vid_frames[4], vid_frames[5],
                vid_frames[6], vid_frames[7], vid_frames[8], vid_frames[9]
            ), 2)
