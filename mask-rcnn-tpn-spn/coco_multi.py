...
import utils
import model_multi as modellib
...
class CocoDataset(utils.Dataset):
    def load_coco(self, dataset_dir, subset, year=DEFAULT_DATASET_YEAR, class_ids=None,
        ...
        if subset == "train":
            coco = COCO("{}/annotations/instance_train_900_clips_dual_cam.json".format(dataset_dir))
        elif subset == "val":
            coco = COCO("{}/annotations/instance_val_100_clips_dual_cam.json".format(dataset_dir))
...
def evaluate_coco(model, dataset, coco, eval_type="bbox", limit=0, image_ids=None):
    ...
    for i, image_id in enumerate(image_ids):
        ...
        t = time.time()
        if len(curr_image) < 9:
            curr_image = [image] + curr_image
            continue
        evaluated_coco_image_ids.append(dataset.image_info[image_id]["id"])
        curr_image = [image] + curr_image
        print("detecting image with image_id = {} and kaggle_id = {}".format(image_id, kaggle_image_ids[i]))
        r = model.detect(curr_image)[0]
        t_prediction += (time.time() - t)
        # rs.append(r)
        # create submission one by one
        print("Finished detecting image with image_id = {} and kaggle_id = {}".format(image_id, kaggle_image_ids[i]))
    ...
    cocoEval.params.imgIds = evaluated_coco_image_ids
    cocoEval.evaluate()
    cocoEval.accumulate()
    cocoEval.summarize()
...
if __name__ == '__main__':
    ...
    if args.command == "train":
        ...
        #dataset_val.load_coco(args.dataset, "train", year=args.year, auto_download=args.download)
